...
export async function setUserLibraryAccess(userId: string, libraryIds: string[]): Promise<void> {
  ...
  await prisma.$transaction([
    ...
    // NOTE: no skipDuplicates — Prisma's SQLite connector doesn't support it (rust-engine/Postgres did),
    // and it's unnecessary here since we just deleted this user's rows and `unique` is de-duplicated.
    ...(unique.length
